...
# from scrapy_deltafetch import DeltaFetchMiddleware
...
class CNNSpider(SitemapSpider):
    ...
    sitemap_rules = [
        ("/[\d]{4}/[\d]{2}/[\d]{2}/[\w-]+/[\w-]+-.*", "parse_article"),
        ("/[\w]+/[\w]+/[\w\d-]+/index.html", "parse_article"),
    ]
    ...
    def sitemap_filter(self, entries):
        for entry in entries:
            ...
            if (
                "live-news" in standard_entry
                or "/video" in standard_entry
                or "/calculator" in standard_entry
                or "/election" in standard_entry
                or "/interactive" in standard_entry
                or "/tv" in standard_entry
                or "/cnn10" in standard_entry
            ):
                continue
            else:
                yield entry
    ...
    #     deltafetch_key = DeltaFetchMiddleware.request_fingerprint(request)

    #     # Set the deltafetch_key meta parameter on the request
    #     request.meta['deltafetch_key'] = deltafetch_key
    ...
    def parse_article(self, response):
        ...
        article_item["tags"] = [
            x.strip() for x in response.xpath('.//meta[@name="keywords"]/@content').get().split(",")
        ]
